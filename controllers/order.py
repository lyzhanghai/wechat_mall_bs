...
class OrderCreate(http.Controller):
    ...
    def post(self, sub_domain, **kwargs):
        try:
            ...
            calculate = True if 'calculate' in kwargs.keys() else False
            couponId = int(kwargs.pop('couponId')) if 'couponId' in kwargs.keys() else None
                ...
                return request.make_response(json.dumps(
                    {
                        ...
                        'data': {
                            ...
                            "amountReal": total,
                            "orderNumber": order_dict['number_goods'],
                        }
                    }))
            else:
                if couponId is not None:
                    user_coupon = request.env(user=user.id)['wechat_mall.user.coupons'].search([
                        ('id', '=', couponId)
                    ])
                    coupon_money = 0
                    if user_coupon:
                        if user_coupon.number > 0:
                            coupon_money = user_coupon.moneyMax
                        if user_coupon.number <= 1:
                            user_coupon.unlink()
                        else:
                            user_coupon.write({
                                'number': user_coupon.number - 1
                            })
                        total -= coupon_money
                        order_dict.update({
                            'total': total
                ...
                return request.make_response(json.dumps(
                    {
                        ...
                        'data': {
                            ...
                            "amountTotle": order.goods_price,
                            "dateAdd": order.create_date,
                            "amountReal": order.total,
                            "orderNumber": order.number_goods,
                        }
                    }))
