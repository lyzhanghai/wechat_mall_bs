...
class GoodsReputationList(http.Controller):
    ...
    def get(self, sub_domain, goodsId):
        try:
            ...
            # 获取商品信息
            goods = request.env(user=user.id)['wechat_mall.goods'].search([
                ('id', '=', goodsId)
            ])
            if not goods:
                return request.make_response(
            ...
            # 获取评价列表
            reputation_list = request.env(user=user.id)['wechat_mall.goods.reputation'].search([
                ('goodsId', '=', goodsId)
            ])

            data = []
            for item in reputation_list:
                data.append({
                    'goods': {
                        'amount': item.amount,
                        'dateReputation': item.dateReputation,
                        'goodReputation': item.goodReputation,
                        'goodReputationRemark': item.goodReputationRemark,
                        'goodReputationStr': defs.GoodReputation.attrs[item.goodReputation],
                    ...
                    'user': {
                        ...
                        'city': user.city,
                        'dateAdd': user.create_date,
                        'dateLogin': user.last_login,
                        'id': user.id,
                        'nick': user.name,
...
class OrderReputation(http.Controller):
    @http.route('/<string:sub_domain>/order/reputation',
    ...
    def post(self, sub_domain, **kwargs):
        try:
            ...
            args_key_set = {'orderId', 'reputations'}
            ...
            reputations = json.loads(kwargs.pop('reputations'))
            for item in reputations:
                new_reputation = request.env(user=user.id)['wechat_mall.goods.reputation'].create({
                    'goodReputation': item['goodReputation'],
                    'goodReputationRemark': item['goodReputationRemark'],
