...
class GoodReputation(models.Model):
    _name = 'wechat_mall.goods.reputation'
    _description = u'商品评价'

    #amount = fields.Integer(string='购买价格')
    dateReputation = fields.Datetime(string='评论时间', default=fields.Datetime.now)
    goodReputation = fields.Selection(defs.GoodReputation.attrs.items(), string='评论等级',
                                      default=defs.GoodReputation.lvl2)
    goodReputationRemark = fields.Text(string='评论')
    #number = fields.Integer(string='购买数量')
    goodsId = fields.Many2many('wechat_mall.goods', string='商品', required=True)
    orderId = fields.Many2many('wechat_mall.order', string='订单', required=True)
    userId = fields.Many2one('wechat_mall.user', string='用户', required=True)
